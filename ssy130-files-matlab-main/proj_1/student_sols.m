...
% directory as this file as well as any of the standard matlab functions.
	...
	function z = add_cyclic_prefix(x,Ncp)  %#ok<*INUSD>
        ...
        % Prepend the cyclic prefix to the original signal
        z = [x(end - Ncp + 1 : end); x];
    end
    ...
    function x = remove_cyclic_prefix(z,Ncp)
        ...
        x = z(Ncp + 1 : end); % Remove the cyclic prefix by discarding the first Ncp samples
    end
    ...
    function bits  = qpsk2bits(x)
     ...
     % Map the symbols to bits
        for i = 1:length(x)
            % Determine the quadrant in the complex plane
            if real(x(i)) > 0
                bits(2 * i - 1) = 1;  % Set the first bit to 1 if real part is positive
            end
            if imag(x(i)) > 0
                bits(2 * i) = 1;  % Set the second bit to 1 if imaginary part is positive
            end
        end

        % Ensure output is of the correct type
    ...
    function [rxPilot, rxData] = split_frame(rxFrame)
        ...
        N = length(rxFrame);
        % Use array indexing to split the frame into pilot and data blocks
        rxPilot = rxFrame(1:N/2);
        rxData = rxFrame(N/2 + 1:end);
    end
    ...
    function [rx, evm, ber, symbs] = sim_ofdm_unknown_channel(tx, h, N_cp, snr, sync_err)
        ...
        % Convert bits to QPSK symbols
        x.p = bits2qpsk(tx.p); % Convert pilot bits to QPSK symbols
        x.d = bits2qpsk(tx.d); % Convert data bits to QPSK symbols
        ...
        % Create OFDM time-domain block using IDFT
        z.p = ifft(x.p); % Create the pilot OFDM block
        z.d = ifft(x.d); % Create the data OFDM block

        % Add cyclic prefix to create OFDM package
        zcp.p = add_cyclic_prefix(z.p, N_cp); % Add cyclic prefix to the pilot block
        zcp.d = add_cyclic_prefix(z.d, N_cp); % Add cyclic prefix to the data block
